...
// Filter for parent annotations (those with child annotations)
def parentAnnotations = allAnnotations.findAll { it.getChildObjects()?.size() > 0 }

println "INFO: Found ${parentAnnotations.size()} parent annotations with child annotations."
...
parentAnnotations.each { parent ->
    ...
    radiusIncrements.each { increment ->
        ...
        newParentAnnotation.setName("${parentName} zero +${increment}")
