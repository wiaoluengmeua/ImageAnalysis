...
GridPane buildPane() {
    ...
    var commandThreshold = new SimpleThresholdCommand(null);
    def transforms = commandThreshold.getAvailableTransforms(imageData)
    commandThreshold = null
   
    def comboChannel = new ComboBox<>(FXCollections.observableArrayList(transforms))
   ...
   pane.add(classificationTittle, 0,row, 2, 1)
   ...
   pane.add(actionsTittle, 0,row, 2, 1)
   ...
   pane.add(runningTittle2, 0,row, 2, 1)
    ...
    GridPaneUtils.setToExpandGridPaneWidth(combo, btnSelected,comboChannel,comboAvobe,comboBelow,comboParent,comboResolutions,comboResolutionsClassifier,comboOutput,btnRunParents)
...
def autoThreshold(annotation, channel, thresholdDownsample, thresholdMethod, thresholdFloor, output, classifierDownsample,prefilterType,prefilterSigma, classBelow, classAbove, minArea, minHoleArea, classifierObjectOptions) {
    ...
    //EZ channel is now directly a ColorTransform
    def classifierChannel = channel
   ...
   //https://github.com/qupath/qupath/blob/4e6bfc31904be7ee39c10d457149cb962002e1f2/qupath-extension-processing/src/main/java/qupath/process/gui/commands/SimpleThresholdCommand.java
   ...
   if(prefilterType == "Gaussian") {
    ...
    //Here we create a new server based on the colortransform
    //https://forum.image.sc/t/imageopserver-not-behaving-as-expected/93422/3
    def imageDataOp = ImageOps.buildImageDataOp( classifierChannel )

    //Here we create start a list of sequential operations. For now we only need the pre-filtering
    List<ImageOp> ops = new ArrayList<>()
    ops.add(prefilter)

    imageDataOp.appendOps( *ops )

    def filteredServer = ImageOps.buildServer( imageData, imageDataOp, cal )    
    ...
    PathImage pathImage = IJTools.convertToImagePlus(filteredServer, RegionRequest.createInstance(filteredServer.getPath(), thresholdDownsample, pathROI)) // Get PathImage within bounding box of annotation
