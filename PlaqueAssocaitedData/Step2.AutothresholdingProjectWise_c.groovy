...
def threshold = 110 // Threshold method
...
double minArea = 50 // Minimum annotation area
double minHoleArea = 0 // Minimum hole area in annotation
String classifierObjectOptions = "SPLIT" // Options for creating classifier objects
...
for (entry in project.getImageList()) {
    ...
    // Find all annotations labeled as "Opal 520"
    def dapiAnnotations = getAnnotationObjects().findAll { it.getName() == "Opal 520" }

    if (dapiAnnotations.isEmpty()) {
        println("No 'Opal 520' annotations found in image: ${entry.getImageName()}")
        continue
    }

    // Apply thresholding to each Opal 520 annotation
    dapiAnnotations.forEach { annotation ->
        println("Applying threshold to Opal 520 annotation in ${entry.getImageName()}")

        autoThreshold(annotation, "Opal 520", thresholdDownsample, threshold, darkBackground, thresholdFloor, output, classifierDownsample, classifierGaussianSigma, null, "Positive", minArea, minHoleArea, classifierObjectOptions)
    }
