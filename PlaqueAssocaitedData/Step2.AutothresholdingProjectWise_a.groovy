...
def threshold = "Triangle" // Threshold method
...
double minArea = 10 // Minimum annotation area
...
for (entry in project.getImageList()) {
    ...
    // Find all annotations labeled as "DAPI"
    def dapiAnnotations = getAnnotationObjects().findAll { it.getName() == "DAPI" }

    if (dapiAnnotations.isEmpty()) {
        println("No 'DAPI' annotations found in image: ${entry.getImageName()}")
        continue
    }

    // Apply thresholding to each DAPI annotation
    dapiAnnotations.forEach { annotation ->
        println("Applying threshold to DAPI annotation in ${entry.getImageName()}")

        autoThreshold(annotation, "DAPI", thresholdDownsample, threshold, darkBackground, thresholdFloor, output, classifierDownsample, classifierGaussianSigma, null, "Positive", minArea, minHoleArea, classifierObjectOptions)
    }
